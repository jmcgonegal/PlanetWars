...
public class PlanetWars {
    ...
    public List<Planet> NotMyPlanets()
    {
        ...
        /*foreach (Planet p in planets)
        {
            ...
            // if I dont own the planet and the future army is 0 then it will be friendly
            if (p.Owner() != 1 && army > 0)
            {
                r.Add(p);
            }
            // I'm going to loose the planet if the ships drop below 0 
            else if (p.Owner() == 1 && army < 0)
            {
                r.Add(p);
            }
        }*/
                           ...
                           int numShips)
    {
        Console.WriteLine("" + sourcePlanet + " " + destinationPlanet + " " +
              numShips);
        Console.Out.Flush();
        /*int ships = this.GetPlanet(sourcePlanet).NumShips() - numShips;
        if (this.GetPlanet(sourcePlanet).Owner() == 1 && ships > 0 && numShips < 30)
        {
            this.GetPlanet(sourcePlanet).NumShips(0);
